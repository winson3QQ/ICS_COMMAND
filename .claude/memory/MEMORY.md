...
- [TAK Marti 授權模型(實測)](tak-marti-authz-model.md) — 讀=truststore 信任即通(註冊無關)/寫=mission-role(MISSION_WRITE)把關；UserAuthenticationFile 非 Marti 讀寫 gate；creatorUid 偽造擋得住；docker 不 hot-reload 須 restart；`/cot/sa?start=&end=&bbox` resync 已驗(#173)；推翻 #176 cert-role 數前提
