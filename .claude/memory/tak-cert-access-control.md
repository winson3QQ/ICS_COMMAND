...
**[2026-06-26 #318 Slice 2/3 全落地 + ❗推翻「live 免重啟」]**：層2 撤銷 3 slice 全 merged（Slice 1 後端 PR #406／Slice 2 provisioning live PR #407／Slice 3 part①②backfill+NULL-fp PR #409、part③④by-fingerprint+SOP PR #410）。`backend-v2.23.0`/`frontend-v1.16.3`。**🔴 真機 iTAK dogfood 推翻上方「DB 撤銷 live 免重啟」**：對**全新/沒連過**的證即時生效（`revtest-318pre`/`revtest-e2e` 先撤再連→當場 `RevokedException`）；**但對「已連過/在線」的證 TAK 按 cert 快取 auth 結果（撐過 client 重連、>5min、撐到 restart）→ 撤了照連，必須 `docker restart takserver` 清快取才即時**（`blue-01` 同證重啟前 `Added Subscription`、重啟後 `RevokedException` 翻轉鐵證）。無更輕清法（`certadmin` 只認 TAK 自發證、force-disconnect `DELETE /subscriptions/delete/{uid}` 回 200 但不清快取、無 cache TTL 設定）。**part④ = 手動 SOP**（ics-command 無 docker.sock、registrar 獨立容器→無安全容器內觸發路徑）。**撤銷盲區（硬限制）**：①TAK API 不吐連線證 hash（`clientEndPoints`/`contacts`/`certadmin` 皆無）→ 不明連線證自動發現無解，靠操作員手貼 fingerprint（part③ `POST /tak/revocations/by-fingerprint`，stdlib `_cert_sha256_fingerprint`==openssl==TAK hash，禁撤 infra 按 hash）②`fingerprint=NULL` 的 #398 前舊證永久撤不掉（UI 標「⚠ TAK 撤不掉」，需重發）。**backfill**：`POST /tak/revocations/backfill` 把 ICS 已撤+有 fp 證一次推進 TAK（補 Slice 2 前只設帳面沒寫 TAK 的洞）。**TAK 自發證（:8446 enrollment）反而易撤**（certadmin 列得出 hash）但本部署 0 張。撤銷意義＝降 `__ANON__`（非硬斷線，靠層1 隔離達 recv 0，兩層必合流）。details 見 ROADMAP P2-26 #318 各 slice 記錄。
