...
- **12c（#229）落地（待 merge/CI）**：`core/database.py` `_connect` driver 抽象 —— `ICS_DB_ENCRYPTED` 旗標：未設→原生 sqlite3（**預設**，dev/CI/Windows 無 wheel/漸進部署）；設→lazy import `sqlcipher3` + `PRAGMA key="x'<DB_KEY 64hex>'"`（DB_KEY = 12a HKDF child[1] db-v1 的 `child.hex()`，raw key 免 KDF；64-hex 白名單驗證）。**收口非單一 `get_conn`**（reality-check 修正原規格假設）：dashboard health ×2（`open_readonly_live`）+ backup online ×2（`online_snapshot`，加密走 `sqlcipher_export` 解明文）一併改。`encrypt_db`/`reencrypt_in_place`（migration + 加密模式 restore re-encrypt，補「備份產物恆明文 .db 包 Fernet vs 加密 live DB」正確性洞）。`scripts/migrate_encrypt_db.py` 明文→加密一次性轉換（idempotent + fail-safe：先驗加密暫存可解才 atomic swap、原明文留 `.pre-encrypt` 備援）。deps `sqlcipher3-binary==0.6.0`（Charles Leifer/coleifer，美國；非中國，security-review 過；Windows 無 wheel → `sys_platform != "win32"` marker）。CI 加「assert sqlcipher3 present」防加密測試靜默 skip + `tests/integration/test_db_encryption.py` 真 SQLCipher round-trip（Windows `importorskip` 跳過、CI ubuntu 實跑）。本機明文 1183 passed、pyflakes 乾淨；加密路徑 CI 驗。**default-off**；§8.4 定位 = **L2 內層、非 at-rest 主控**（完整 at-rest 需整碟 #231，是否真開 + 交付模式待 #231 前置定案）。security-review（da8e492/a0b85b8 diff）0 finding。
