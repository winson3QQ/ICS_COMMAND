...
| P2-24 | **TAK 連結 runtime 開關（[issue #164](https://github.com/winson3QQ/ICS_COMMAND/issues/164)，2026-06-07 P2-10 dogfood 衍生）**：現況 TAK 連線由啟動時 `TAK_ENABLED` env 決定、改需**重啟 process**；指揮部應能 **runtime 啟用/停用** subscribe，不重啟。落地：(1) `COMMAND_ROLES` 限定開關 `POST /api/tak/connection {enabled}`；(2) 後端能 start/stop subscribe 背景 task（不重啟 process）；(3) 狀態持久化（重啟後維持選擇）。**安全**：開/關 TAK 連線＝指揮行為 → **強制 audit**（`TAK_CONNECTION_TOGGLE operator={uid} enabled={bool}`，不得 best-effort）；`COMMAND_ROLES` 嚴格限（403 負向必通）。依賴 P2-23（狀態顯示）+ P2-03。 |
