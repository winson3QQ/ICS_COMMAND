...
| 🚧 P2-11b | **CoPEntity schema 擴充**：(1) `source` enum 加 `"command"`（P1-03 解凍）；(2) `planned: bool`（空心/實心 2525 框，P2-13）；(3) `simulated: bool`（合成注入標記，`how="h-g-i-g-o"`，P2-19）。<br>**[planned/simulated 完成 — [#143](https://github.com/winson3QQ/ICS_COMMAND/pull/143)（issue #140），分階段 B]**：reality check 發現 source enum 加 `'command'` 撞 SQLite 結構性坑（`source` 有 CHECK 不可 ALTER → 須 table rebuild → DROP parent 觸發 tracks/links `ON DELETE CASCADE` 刪光、`foreign_keys=OFF` 在 transaction 內無效）。**不把零風險 ADD 卷進高風險 rebuild** → 本批只做 `planned`/`simulated`（`_m017` `ADD COLUMN` INTEGER 0/1 + repo int↔bool、schema bool default False）；**source `'command'` rebuild 拆出延 [#141](https://github.com/winson3QQ/ICS_COMMAND/issues/141)（P2-13 前置，含 `legacy_alter_table` rebuild SOP + planned/simulated 不可被 untrusted CoT 偽造的 security wiring 守則）**。測試 schema/migration/bool roundtrip + migration chain 動態化；**完整 suite 全綠**。<br>**就緒度**：P2-13 用的 `planned`、P2-19 用的 `simulated` 欄位**已就緒**；P2-13 動工前須先解 [#141](https://github.com/winson3QQ/ICS_COMMAND/issues/141)（source `'command'`）|
...
| # | 缺口 | 現況 | 影響 | 決策方向 |
|---|---|---|---|---|
| 1 | **CoPEntity `source` enum 凍結** | `schemas/cop.py:21`：`Literal["manual","pi-node","tak","waveink"]`（P1-03 凍結）；加 `"command"` 需 migration | ~~P2-11b~~ → **[#141](https://github.com/winson3QQ/ICS_COMMAND/issues/141)** | P2-11b reality check 發現加 enum 值撞 SQLite CHECK 不可 ALTER + FK cascade → 拆出延 #141（P2-13 前置，`legacy_alter_table` rebuild） |
...
- [ ] CoPEntity `source` enum 加 `"command"`（table rebuild，SQLite CHECK 不可 ALTER）（[#141](https://github.com/winson3QQ/ICS_COMMAND/issues/141)，P2-13 前置）
