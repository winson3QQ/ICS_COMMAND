...
| P1-02 | `routers/ingress.py`（自 `pi_push.py` 改名）為通用 ingress 路由集合；目前承載 pi-node（P3 將加 waveink，TAK 留在 `routers/tak.py`）。新主路徑 `/api/ingress/pi-node/{unit_id}`；舊 `/api/pi-push/{unit_id}` 以同 handler 雙裝飾器保留為別名（ICS_DMAS Pi client 向後相容） |
| P1-02b | **Client-side 遷移到 `/api/ingress/pi-node/{unit_id}`**：本 repo `server/sync.js`（L187/L217/L248）+ 外部 ICS_DMAS Pi node client 仍硬寫舊別名 `/api/pi-push/{unit_id}`。本地 client 直接改即可；ICS_DMAS 需上游 PR 協同。**完成此 item 才能評估移除別名**（否則 server-side 拆別名 = 整條 federation push 404）。alias 為長期保留意圖，移除非急務，但此 item 是任何「拆別名」action 的前置 |
