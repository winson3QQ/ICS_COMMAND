...
| P2-33 | **event↔marker 前端 cutover（P2-27 拆出，JSON glue 退役）**：#183 已建 `event_markers` junction 為**權威關聯** + 導航端點 `GET /api/events/{id}/chain`，但**前端仍走 `attributes.event_id` JSON glue**、且 junction / `/chain` **無前端消費者**（故 #183 無使用者可見變化）。工作：前端改消費 junction / `/chain`（事件↔標記關聯、導航鏈 UI）+ 拔掉建 event 時塞 `attributes.event_id` 的 glue（`map.js` `_evPopupSubmit`）→ junction 成**唯一**關聯來源。**這是 #183 後端地基真正產生使用者可見價值的一刀。** 依賴 P2-27（#183）。**📐** [`cop-event-layering.md`](design/cop-event-layering.md)。 |
