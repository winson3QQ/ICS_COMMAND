...
| P2-10 | **地基層驗收 + E2E 上行測試**：地基層（P2-06a～P2-11b）完成後的整合驗收里程碑。涵蓋：(1) 真實 ATAK/iTAK 推 CoT → 地圖 < 5s 更新（**human verify**）；(2) shape 幾何渲染（route/polygon **human verify**）；(3) mock TAK integration tests；(4) CoT 內容層驗證（座標越界拒絕、callsign 字元白名單、type prefix 白名單——任何 ATAK 裝置可推 CoT，**此為最後一道防線**）；(5) XXE 防護驗測（`defusedxml`）；(6) GeoChat 路由負向（不進 `cop_entities`）。**全通後才進 P2-12/P2-13**。<br>**[進度 2026-06-07]** 純後端項先行：✅ (4) 內容層 type/callsign 白名單（[#157](https://github.com/winson3QQ/ICS_COMMAND/issues/157)，`CoTEventIn` field_validator，:8089+REST 兩路徑；座標越界 P2-04 已擋）；(5) XXE 已於 P2-02 `defusedxml`、(6) GeoChat 負向已於 P2-07 落地。**剩 (1)(2) 真機 human verify + (3) mock 整合 —— 待 TAK server 上線（2026-06-07 Windows Docker 部署中）** |
