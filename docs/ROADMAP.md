...
| **TAK-E** ⏳ **時機重歸屬（2026-06-07）** | `:8089` 串流訂閱（`subscribe()`）無流量管制；每 uid 有 1s 精度保護（`_normalize_iso8601` 秒 floor + `_is_newer` 比較），但 uid 數量無上限；50 ATAK @ 1Hz = 50 DB write + 50 WS broadcast/s 持續壓 | DoS 弱面（多 uid 高頻 burst）；單 uid 已有 1s 保護，uid 爆量未擋 | ~~P2-06a 同批~~ **P2-06a（[#121](https://github.com/winson3QQ/ICS_COMMAND/pull/121)）已 merged，當時聚焦軌跡寫入未含此項** → 重歸屬 **P2-10（地基層 E2E 驗收）或獨立 hardening PR**。**怎麼動**：per-exercise uid count 上限（configurable env）+ global async token bucket（如 60 events/s）；超量 `log.warning` 不中斷串流 |
