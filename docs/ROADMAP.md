...
| ⏳ P2-02 | `services/tak_service.py`：CoT XML 解析（規格相容，禁止自創欄位）；TAK Server 推播訂閱（TCP/SSL 8089 或 federation port 9000）。**[Wave 0+1 完成 — [#103](https://github.com/winson3QQ/ICS_COMMAND/pull/103) `d5deaad`（issue #102）]**：`parse_cot_xml()` XXE-safe（defusedxml forbid_dtd/entities/external）+ `CoTEventIn` 移 `schemas/tak.py` 補 CoT 必填 start/how/version（修 ↔CoPEntity contract gap）+ 時間戳 normalize 秒精度 Z（對齊 stale 字典序）+ detail 抽 callsign/remarks/結構化 dict；15 測試（unit/security/XXE/billion-laughs/大小上限）。/code-review 2 修正、/security-review 0 finding。**Wave 2（8089 mTLS 訂閱）未做**：reality-check 確認 8089 = TAK Protocol（v0 XML/v1 protobuf，非純 XML）+ 強制 client cert（實機握手實測 mTLS）→ 用 pytak[with-takproto]（snstac，Apache2/MIT，非中國）；前置＝擴充 `issue-tak-certs.sh` 簽 **client** cert（現只簽 server/fed-truststore）。活 TAK server 已就緒（:8089 listen）。 |
