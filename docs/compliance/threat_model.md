...
| **TAK-B** | Tampering | TAK CoT uid 直通，可偽造/碰撞 `manual:*` 等本地他源 uid → `ingest_cot_event` CAS 覆寫本地 entity | **✅ 已修（[#145](https://github.com/winson3QQ/ICS_COMMAND/issues/145)，2026-06-07）：來源所有權守門**。CoT 標準要求轉傳保留 uid 不改寫（供 P2-13 下行對位）→ 不前綴改名；改為 TAK 事件只准動 `source='tak'`，撞本地他源一律拒絕覆寫、回 None 記 log（`cop_service.ingest_cot_event` + 負向測試）。**✅ 反向亦已修（[#146](https://github.com/winson3QQ/ICS_COMMAND/issues/146)，2026-06-07）**：cop `PUT`/`DELETE` 加來源所有權 + 情境守門（`_require_editable_source`）—— `manual`/`command` 永遠可編輯；`tak`/pi-node/waveink **僅演習(TTX)模式可編輯**（server 權威，依 active exercise type），**實戰模式鎖死**（保護真實位置不被造假/誤刪；下令走 `source='command'` 新物件）。並順手關注入後門：POST /api/tak/* 收緊 COMMAND_ROLES（見 TAK-A）。**⚠️ [2026-07-03 #474（β-2）推翻「實戰鎖死」]**：使用者 2026-06-14 拍板「COP＝共享真實、協作可編」→ **`_require_editable_source` TTX-only 閘已拆除**，外部來源（tak/pi-node/waveink）幾何 + marker 在**任何模式（TTX/實戰/平時）皆可編/刪**。**威脅改由問責緩解、非模式鎖**：① **audit**（PUT→`cop_entity_updated` / DELETE→`cop_entity_deleted`，恆留痕、可回溯誰改了什麼）；② **RBAC**（限 WRITE_ROLES，observer 不可）；③ **`_PUT_FORBIDDEN_FIELDS`**（擋 source/uid/version_clock 竄改，身分不可偽）；④ **P2-10 內容白名單**（座標/callsign/type 越界拒）；⑤ **編輯不自動回推 TAK**（走明示「📡 廣播」）→ ICS 端更正≠立即污染 TAK 前線真相。**#145 來源所有權（TAK 不可覆寫本地他源 uid）不受影響、仍在**。`/security-review` 背書。詳見 memory `cop-collaborative-edit-shared-truth` |
...
- **層2（點名封殺）= 撤銷**（[#318](https://github.com/winson3QQ/ICS_COMMAND/issues/318)）：ICS **離線簽**的證 TAK `certificate` 表**查無** → 預設**撤不掉**（`findOneByHash` 撲空）；須 ICS 發證後**把證補登進 TAK DB** + 開 `x509checkRevocation`。**實證**：手插 `certificate`(hash+revocation_date) + 開旗標 + restart → 該證連 :8089 立刻被踢（recv 0）、REST 500，未撤的 `ics-cot` 正常。
  <br>**[2026-06-26 #318 Slice 2 — live enforce 接通 + reality check 校正（backend-v2.21.0）]**：`x509checkRevocation="true"` 入 prod CoreConfig（git-tracked、bind-mount 持久）+ ICS `TAK_DB_*` env 接通 → `services/tak_revocation.revoke_in_tak` 由 dashboard 撤銷時**直寫** `certificate` 表（按 fingerprint 精準、infra 護、best-effort）。**活機端到端校正三點**（推翻「手插+restart=立刻被踢」的過簡描述）：(a) **fail-open 安全**——`certificate` 表空時開旗標，`ics-cot`/iTAK（離線 CA、表內無 row）restart 後照常重連串流（無 row=放行，不會誤鎖合法證）；(b) **撤銷 ≠ 硬斷線**——被撤證仍因 CA 受信落 `__ANON__`（`RevokedException` 後仍 `Added Subscription`），靠**層1 隔離**才達 recv 0（§8.3 兩層**必須合流**，非各自完備）；(c) **快取**——撤銷對「尚未認證過」的證**即時生效（免 restart）**（`revtest-318pre` 先撤再連 → 當場 `RevokedException`），但對「**在線、近期已認證**」的證 TAK 快取其有效性（實測 >5min、極可能撐到 restart）→ 要即時踢在線證須**重啟 TAK 清快取**（業主拍板：當網路不穩定的一部分，client 自動重連，可接受）。
