...
# MINOR 2.31.0：#474（β-2）拆 TTX-only 編輯閘——外部來源（tak/pi-node/waveink）幾何 + marker 在任何模式
...
# MINOR 2.38.0：#507 ICS TAK 身分模型「兩面一軸」——infra 證群歸屬宣告式 SoT（services/tak_identity）+
...
CMD_VERSION: str = os.getenv(
    ...
    # PATCH v1.24.1：#473-B3 開場精靈 overlay 修正（iPad dogfood）——精靈用通用 modal（z=210）被演習
