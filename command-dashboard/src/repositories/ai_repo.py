...
def create_recommendation(
    exercise_id: int | None, recommendation_type: str, content: str, confidence: float | None = None
) -> dict:
    now = now_utc()
...
def update_outcome(
    ...
    with get_conn() as conn:
        cur = conn.execute(
            "UPDATE ai_recommendations SET accepted=?, related_decision_id=?, outcome_notes=? WHERE id=?",
...
def get_recommendations(exercise_id: int) -> list[dict]:
    with get_conn() as conn:
        rows = conn.execute(
            "SELECT * FROM ai_recommendations WHERE exercise_id=? ORDER BY made_at DESC", (exercise_id,)
        ).fetchall()
