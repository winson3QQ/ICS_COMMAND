...
def execute_three_pass(source_unit: str, sync_data: dict, operator: str = "auto") -> dict:
    """三 Pass 對齊邏輯（斷網恢復同步）"""
    ...
    with get_conn() as conn:
        ...
        for snap in sync_data.get("snapshots") or []:
            ...
            existing = conn.execute("SELECT id, source FROM snapshots WHERE snapshot_id=?", (snap_id,)).fetchone()
...
def get_sync_log(source_unit: str | None = None, limit: int = 20) -> list[dict]:
    with get_conn() as conn:
        if source_unit:
            rows = conn.execute(
                "SELECT * FROM sync_log WHERE source_unit=? ORDER BY sync_started_at DESC LIMIT ?", (source_unit, limit)
            ).fetchall()
        else:
            rows = conn.execute("SELECT * FROM sync_log ORDER BY sync_started_at DESC LIMIT ?", (limit,)).fetchall()
    result = []
    for r in rows:
        d = row_to_dict(r)
...
def resolve_conflict(sync_id: str, conflict_idx: int, action: str, operator: str) -> dict:
    ...
    audit(operator, None, "conflict_resolved", "sync_log", sync_id, {"conflict_idx": conflict_idx, "action": action})
    return {"sync_id": sync_id, "conflict_idx": conflict_idx, "action": action, "status": new_status}
