        ...
        snap.update(
            {
                ...
                "waiting_count": len(
                    [p for p in active if p.get("care_status") == "triaged" and p.get("disposition", "在場") == "在場"]
                ),
...
def build_dashboard(exercise_id: int | None = None) -> dict:
    ...
    events = sorted(open_ev + prog_ev + res_ev, key=lambda e: e["occurred_at"], reverse=True)
    ...
    return {
        ...
        "decisions": {
            "pending": pending,
            "decided": sorted(approved + completed, key=lambda d: d.get("decided_at", ""), reverse=True)[:20],
            "chains": chains,
        },
