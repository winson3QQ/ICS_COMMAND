...
def download_tiles(zoom_min, zoom_max):
    ...
    for zoom in range(zoom_min, zoom_max + 1):
        ...
        for x in xs:
            for y in ys:
                row = db.execute(
                    "SELECT 1 FROM tiles WHERE zoom_level=? AND tile_column=? AND tile_row=?", (zoom, x, tms_y(y, zoom))
                ).fetchone()
                if row:
                    done += 1
                    continue
                ...
                try:
                    ...
                    with urllib.request.urlopen(req, timeout=10, context=SSL_CTX) as resp:
                        data = resp.read()
                    db.execute("INSERT OR REPLACE INTO tiles VALUES(?,?,?,?)", (zoom, x, tms_y(y, zoom), data))
                    db.commit()
