...
class TestReplayDuplicateSync:
    ...
    def test_duplicate_does_not_increase_db_count(self, tmp_db):
        ...
        with get_conn() as conn:
            cnt = conn.execute("SELECT COUNT(*) as c FROM snapshots WHERE snapshot_id='replay-test-001'").fetchone()[
                "c"
            ]
        assert cnt == 1
    ...
    def test_duplicate_via_api_both_return_200(self, hmac_client):
        ...
        snap = {
            "v": 3,
            "type": "shelter",
            "snapshot_id": "api-replay-001",
            "t": "2026-04-24T09:00:00Z",
            "src": "test",
            "bed_used": 5,
            "bed_total": 50,
        }
...
class TestWithinPayloadDuplicate:
    def test_duplicate_snapshot_id_in_single_sync_payload_last_wins(self, hmac_client):
        ...
        with get_conn() as conn:
            rows = conn.execute("SELECT bed_used FROM snapshots WHERE snapshot_id='within-payload-001'").fetchall()
        assert len(rows) == 1
...
class TestOutOfOrderSync:
    def test_latest_reflects_snapshot_time_not_arrival(self, tmp_db):
        ...
        from repositories.snapshot_repo import get_latest_snapshot, upsert_snapshot
