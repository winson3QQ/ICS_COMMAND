...
def test_conn_wants_filter_matrix():
    ...
    # 指定 exercise=1 → 收 1 與全域(None)，不收別的
    assert _Conn(None, 1).wants(1) is True
    assert _Conn(None, 1).wants(None) is True
...
def test_global_entity_reaches_exercise_subscriber():
    async def run():
        hub = CopHub()
        sub = _FakeWS()
        await hub.connect(sub, 1)
        await hub.broadcast({"m": "global"}, exercise_id=None)  # 無 exercise 歸屬
        assert sub.sent == [{"m": "global"}]
