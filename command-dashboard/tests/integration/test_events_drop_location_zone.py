...
- EventIn schema 對殘留 location_zone_id 容忍（extra ignored，不 422）——
  前端已停送（P2-32 前端尾收尾 #186），後端容忍保留為對舊 / 快取 client 的防禦
...
class TestCreateEventNoZoneCol:
    def test_create_ignores_stray_location_zone_id(self, tmp_db):
        """data dict 殘留 location_zone_id（舊 / 快取 client）→ create 忽略、不炸。"""
    ...
    def test_patch_ignores_location_zone_id(self, tmp_db):
        ...
        from repositories.event_repo import create_event, get_events, patch_event
...
class TestSchemaBackCompat:
    def test_eventin_ignores_extra_location_zone_id(self, tmp_db):
        """舊 / 快取 client 仍可能在 POST body 送 location_zone_id（前端已停送）→
